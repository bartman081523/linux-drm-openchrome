			...
			u8 *buffer, unsigned int size)
{
    ...
    if (ret != 2) {
        dev_warn(dev->dev, "%s: i2c transfer failed: %d\n", __func__, ret);
        return;
    }
			...
			u8 *data, unsigned int size)
{
    ...
    if (!out_buf) {
        dev_err(dev->dev,
            "%s: out_buf allocation of %u bytes failed\n",
            __func__, size + 1);
        return;
    }
    ...
    if (ret != 1) {
        dev_warn(dev->dev, "%s: i2c_transfer failed: %d\n", __func__, ret);
        goto exit;
    }
...
/*
 * Old Platforms I2C register initialization.
 */
